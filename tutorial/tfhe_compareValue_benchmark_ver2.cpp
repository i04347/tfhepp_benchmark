...
//    std::system("(iotop -b -d 0.5 -k >> test.txt)&");
   ...
   */
    ...
    std::vector<TFHEpp::TLWE<TFHEpp::lvl0param>> ciphertext(vector_size), ciphertext2(vector_size-1);
    TFHEpp::TLWE<TFHEpp::lvl0param> check, res;
    std::cout << "TFHEpp::lvl0param n is " << TFHEpp::lvl0param::n << std::endl;
    ...
    res = TFHEpp::tlweSymEncodeEncrypt<TFHEpp::lvl0param>(
            0, TFHEpp::lvl0param::alpha, sk->key.lvl0, encoder);
   ...
   */
    ...
    for (int i = 0; i < vector_size-1; i++) {
       ...
       // TFHEpp::HomAND(next, prev, next, *gk.get());
        ...
        TFHEpp::HomSUBFixedEncoder(ciphertext2[i], ciphertext[i+1], check, encoder, encoder);
        TFHEpp::HomADDFixedEncoder(res, ciphertext2[i], res, encoder, encoder);

    }
    TFHEpp::ProgrammableBootstrapping(res, res,
                                         *gk.get(), encoder, encoder,
                                          identity_function);
