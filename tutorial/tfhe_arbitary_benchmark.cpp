...
int main()
{
    std::system("(top -b -d 0.05 >> top_arbitary_2.txt)&");
...
//    std::system("(iotop -b -d 0.5 -k >> test.txt)&");
   ...
   */
    ...
    for (int i = 0; i < vector_size-1; i++) {
       ...
       TFHEpp::HomMULT(ciphertext2[i+1], ciphertext[i+1], ciphertext2[i], *gk.get(), encoder_bs, encoder_bs, encoder_bs);
    ...
    decrypted_answer = TFHEpp::tlweSymDecryptDecode<TFHEpp::lvl0param>(
            ciphertext2[vector_size-1], sk->key.lvl0, encoder_bs);
    //for (int i = 0; i < vector_size; i++) {
