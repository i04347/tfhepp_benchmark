...
void FullMulter(TFHEpp::TLWE<TFHEpp::lvl0param>& res,
...
int main()
{
    std::system("(top -b -d 0.05 >> top_innerproduct_2.txt)&");
...
//    std::system("(iotop -b -d 0.5 -k >> test.txt)&");
   ...
   */
    double encoder_a =-60; //-100.
    double encoder_b = 60.;
    TFHEpp::Encoder encoder_bs(encoder_a, encoder_b, 32);
    ...
    std::vector<TFHEpp::TLWE<TFHEpp::lvl0param>> ciphertext(vector_size), ciphertext2(vector_size);
    std::cout << "TFHEpp::lvl0param n is " << TFHEpp::lvl0param::n << std::endl;
    double encryption_start = get_time_msec();
    for (int i = 0; i < vector_size; i++) {
        ciphertext[i] = TFHEpp::tlweSymEncodeEncrypt<TFHEpp::lvl0param>(
            p[i], TFHEpp::lvl0param::alpha, sk->key.lvl0, encoder_bs);
    }
    //std::cout << "test" << std::endl;
    //for (int i=0; i<TFHEpp::lvl0param::n; i++){
    //    std::cout << ciphertext[0][i] <<std::endl;
    //}
   ...
   */
    ...
    TFHEpp::TLWE<TFHEpp::lvl0param> res = TFHEpp::tlweSymEncodeEncrypt<TFHEpp::lvl0param>(
            0, TFHEpp::lvl0param::alpha, sk->key.lvl0, encoder_bs);
    start = get_time_msec();
    //ciphertext2[0] = ciphertext[0];
    for (int i = 0; i < vector_size; i++) {
       ...
       TFHEpp::HomMULT(ciphertext2[i], ciphertext[i], ciphertext2[i], *gk.get(), encoder_bs, encoder_bs, encoder_bs);
       TFHEpp::HomADDFixedEncoder(res, res, ciphertext2[i], encoder_bs, encoder_bs);
        //TFHEpp::HomADD(res, res, ciphertext2[i+1], encoder, encoder);
        //FullMulter(ciphertext2[i+1], ciphertext[i+1], ciphertext2[i], *gk.get());
    }
